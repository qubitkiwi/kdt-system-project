...
{
	int left_led_value = OFF;

	pr_info("toy simple io test 1\n");
	while (1) {

		if (left_led_value == ON) {
			left_led_value = OFF;
...
{
	int right_led_value = OFF;

	pr_info("toy simple io test 2\n");
	while (1) {
		if (right_led_value == ON) {
			right_led_value = OFF;
...
////////
int gpio_input_edge_down = 0;
DECLARE_WAIT_QUEUE_HEAD(wait_edge_queue);

static irq_handler_t gpio_irq_down_edge_handler(unsigned int irq, void *dev_id, struct pt_regs *regs)
{
	pr_info("interrupt triggered\n");
	gpio_input_edge_down = 1;
	wake_up_interruptible(&wait_edge_queue);
...
{
	pr_info("toy simple io test 6\n");
	int toggle = 9;
	int input_value = 0;
	wait_queue_flag = WAIT_QUEUE_WAIT;

	unsigned int button_irq = gpio_to_irq(TOY_GPIO_INPUT);
	gpio_set_debounce(TOY_GPIO_INPUT, 300);

	if (request_irq(button_irq, (irq_handler_t) gpio_irq_down_edge_handler, IRQF_TRIGGER_FALLING, "kdt_gpio_irq_signal", NULL) != 0) {
		pr_err("Error!\n Can not request interrupt nr: %d\n", button_irq);
		return -1;
	}

	while (1) {
		gpio_input_edge_down = 0;
		wait_event_interruptible(wait_edge_queue, gpio_input_edge_down == 1);
		input_value = gpio_get_value(TOY_GPIO_INPUT);
		if (input_value == 1)
			continue;
...
MODULE_DESCRIPTION("simple gpio");
